    ...
    for i, (pts, branch, brlens, conn) in enumerate(
        ...
        for j, (br_pts, br_conn) in enumerate(zip(separated_pts, conn)):
            ...
            for c in br_conn:
                ...
                '''
                The units in this don't work out for some reason??? Why is layer_sep converted to microns, but pixel width is nm. 
...
def get_hacha_resistance(struct, single_pixel_width=50.0):
    """Gets the resistance for each slice and for each point in a slice.
    But with funkiness to get f3ast to treat the center of stl
